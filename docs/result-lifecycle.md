...
## Implemented historical snapshot repair

Owners open the result review’s accepted-evidence repair page, select a squad and review the latest accepted pre-deadline choice, full original/reconstructed fields and canonical ranking/prize effects. No manual replacement lineup or older receipt can be supplied. Missing, late or ambiguous evidence blocks repair. Confirmation atomically retains a repair version, reopens and publishes results with a new correction window. Old publications resolve their own snapshot version; current scoring, later rules replays and public pitches resolve the repaired version. Original records and subsequent account decisions remain intact. Private game-data exports include owned repair scoring fields without staff notes/source internals. See [slice 39](implementation-slice-39.md).
