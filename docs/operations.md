...
A pause prevents new reservations. Already reserved short leases or in-flight requests may finish and remain charged. Changing season settings cancels pending batches; restore recovery still requires pausing and reconciling the provider account before worker restart. Collected responses do not automatically become accepted football facts or scores. The current source adapter requires a reviewed ordinary-full-time match report; automatic validated ingestion and the provisional-publication latency target remain unproven.
