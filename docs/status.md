...
Provider discovery: the supplied Free key authenticates but rejects 2026 (allowed 2022–2024). Eight diagnostic HTTP calls have been made, including two status calls. The last response reported 95 daily requests remaining; quota headers are observations, not permission to refund local usage. One historical 2024 match bundle was held for lineup/statistics disagreement and a VAR event; no real report was imported. The verified checkpoint is on GitHub with protected, tested PR merges. Automatic collection remains off. See [account evidence and next steps](provider-account-validation-2026-09-19.md) and [valuation sourcing](market-valuation-sourcing.md).

See [prize correction projections](implementation-slice-35.md), [classic/H2H correction work](implementation-slice-34.md), [release execution order](release-execution-plan.md) and [slice 33](implementation-slice-33.md) for the Git checkpoint, mandatory CI and reviewed match-timeline work.
...
- Reviewed void/replay/awarded fixture dispositions, retained original evidence, cumulative resumption and explicit zero-performance settlement with atomic publication and preserved chips/transfer deductions (slice 38).
- Owner-authorized historical scoring-rule corrections with exact preview, atomic rule/result publication, preserved participant decisions, retained calculations and dynamic bilingual playbooks.
- Overall/classic-league/H2H correction projections with shared published calculations, private-group redaction, hypothetical prize allocations with scoped financial access, immutable recorded award comparisons and stale-confirmation protection across result/group/edition/prize/eligibility dependencies.
...
- Reviewed provider match drafts with source/mapping retention, explicit historical eligibility evidence and unchanged-observation deduplication. Version-2 ordinary FT timelines derive evidenced goal counts, substitutions, defensive concessions, cards and penalty events while holding ambiguities and incomplete source data.
...
The complete launch scope is **not finished**. Ordinary rules, Cairo date controls, catalogue/valuation editing, reviewed performance-price batches and the persistent scored rehearsal are implemented. Governed automatic FT report acceptance is implemented in [slice 40](implementation-slice-40.md). Continue licensed provider-specific validation, real-data calibration and policy approval, retention/exceptional privacy handling and production rollout and measured recovery. Owner-only, evidence-backed historical participant snapshot repair is implemented in [slice 39](implementation-slice-39.md), with immutable history, coherent publication and private export coverage. Exceptional fixture handling and explicit empty-round settlement are implemented in [slice 38](implementation-slice-38.md). The full remaining work is tracked in [production readiness](production-readiness.md). [Slice 37](implementation-slice-37.md) adds owner-authorized historical scoring correction with atomic publication and bilingual impact review. [Slice 36](implementation-slice-36.md) adds pre-write cross-competition match review and a bilingual home scoreboard with winning formations, full rankings and published-data refresh. See [slice 23](implementation-slice-23.md), [slice 24](implementation-slice-24.md), [slice 25](implementation-slice-25.md), [slice 26](implementation-slice-26.md), [slice 27](implementation-slice-27.md), [slice 28](implementation-slice-28.md), [slice 29](implementation-slice-29.md), [slice 30](implementation-slice-30.md) and [implementation audit](implementation-audit.md) and [delivery plan](delivery-plan.md).

A staging host/domain is still needed. Provider/valuation licensing and target-season coverage, production credentials, calibration and measured load/restore/cost evidence remain [explicit gates](evidence-register.md). The supplied remote contains the verified baseline; main requires current-head CI and secret scanning, including for administrators. No paid infrastructure or staging/production deployment has been created.

## Verification

`pnpm check` covers formatting, typed lint, strict source/test checks, production build and 113 domain/authorization/date/request/provider/guide tests plus nine authentication/backup/release harness tests. `pnpm test:integration` runs ten persistence cases and 142 application cases/subcases on disposable PostgreSQL. `pnpm test:smoke` checks 56 public page combinations across language/theme/viewport, plus access/readiness boundaries. `pnpm test:e2e` (also `test:browser`) uses local Chrome and real PostgreSQL/authentication, with disposable account/competition/fixture cleanup. `pnpm test:containers` verifies isolated Linux startup, migrations, read-only operation and durable jobs; image proof is in `artifacts/verification/container-proof.json`. Screenshots are under `artifacts/brand` and `artifacts/web`. `pnpm verify:local` runs checks/integrations before starting an owned preview for smoke/E2E, preventing rebuilds during browser verification; see [slice 31](implementation-slice-31.md).
...
[Slice 41](implementation-slice-41.md) adds encrypted PostgreSQL base/WAL backups, operations/timer templates, a source-bound clean-volume PITR drill and writable-primary readiness. `pnpm backup:build` and `pnpm test:recovery` exercise the separate backup image; required CI now includes the drill. Actual off-host access, failure alert delivery and production-sized recovery remain deployment evidence gates.

[Slice 42](implementation-slice-42.md) adds pinned production HTTPS, private backend ports, release preflight and real production-authentication tests through Caddy. It fixes authentication initialization after premature readiness probes and verifies secure sessions, origin/rate-limit boundaries and private outage logs. Public ACME/DNS/email and selected-host evidence remain open.

[Slice 43](implementation-slice-43.md) proves 200 queued late requests are rejected, partial deadline snapshots roll back on backend termination and competing workers recover 200 squads exactly once. Managed PostgreSQL pools prevent idle/checked-out connection error events from crashing web/worker processes; container tests cover database restart, session/rate-limit preservation and subsequent durable work. Full selected-host HTTP capacity remains unmeasured.
